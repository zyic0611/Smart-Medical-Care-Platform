    ...
    public static SysUser getCurrentUser() {
        try {
            ...
            if (StrUtil.isNotBlank(token)) {
                ...
                // (注意：validateToken 可能会抛异常，这里 catch 住返回 null 即可)
                String userId = validateToken(token);


                // 4. 使用静态 Mapper 去数据库查询完整的用户信息
                // (必须用 staticEmployeeMapper，不能用 employeeMapper)
                return staticSysUserMapper.selectById(Integer.valueOf(userId));
            }
        } catch (Exception e) {
            // 只要出问题（没Token、Token过期、解析失败），都视为未登录
            return null;
        }
        return null;
    }
